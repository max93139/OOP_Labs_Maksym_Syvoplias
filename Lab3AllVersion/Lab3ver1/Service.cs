...
namespace Lab3
{
    ...
    public class Service
    {
        ...
        {
            return $"  #{s.StudentNumber} {s.Name}  Спеціальність : {s.EducationProgram} " +
                   $" Кредити: {s.WorkloadLevel} " +
                   $" Рейтинг: {s.Rating:F2}  Оцінки: [{string.Join(", ", s.Grades)}] " +
                   $" Причина: {(!string.IsNullOrEmpty(s.AdmissionReason) ? s.AdmissionReason : "не вказано")}";
        }
        ...
        private void HandleAddGrade(Department department)
        {
            ...
            while (addingGrades)
            {
                int grade = ReadInt($"  Оцінка для «{s.Name} (0–100, -1 — завершити): ");
                if (grade == -1)
                {
                    addingGrades = false;
                    continue;
