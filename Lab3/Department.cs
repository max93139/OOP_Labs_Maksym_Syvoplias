...
namespace Lab3
{
    ...
    public class Department
    {
        ...
        /// <summary>Форматує інформацію про кафедру.</summary>
        public string ToFormattedString()
        {
            ...
            return $"Кафедра: {name}\n" +
                   $"Спеціальність : {educationProgram}\n" +
        ...
        //  Методи управління (Сценарій тепер у Program.Main)
        ...
        public void PrintStudentList(Service svc)
        {
            ...
            {
                svc.WriteToConsole($"  {i + 1}. {students[i].Name}");
            }
        }
        ...
        /// <summary>Показує список спеціальностей та повертає вибрану.</summary>
        ...
        /// <summary>Заповнює дані існуючої кафедри через консоль.</summary>
        public void ReadFromConsole(Service svc)
        {
            svc.WriteToConsole("  Назва кафедри: ");
            this.name = svc.ReadFromConsole();
            svc.WriteToConsole("  Спеціальність  підготовки: ");
            this.educationProgram = svc.ReadFromConsole();
            this.maxStudents = svc.ReadInt("  Максимальна кількість студентів: ");
        }
        ...
        /// <summary>Ініціалізація або оновлення даних існуючої кафедри.</summary>
        public void HandleCreate(Service svc)
        {
            ReadFromConsole(svc);
            svc.WriteToConsole($"   Дані кафедри «{this.name}» успішно оновлено.");
        ...
        public int HandleAddStudent(Service svc, int nextStudentNumber)
        {
            ...
            else
            {
                if (AddStudent(newStudent))
                {
                    ...
                    nextStudentNumber++;
                    svc.WriteToConsole($"   Студента «{newStudent.Name}» додано (Причина: {reason}). Всього: {studentCount}/{maxStudents}");
                }
                else
                {
                    svc.WriteToConsole("   Досягнуто максимальну кількість студентів або невірна спеціальність!");
                }
            }
        ...
        public void HandleRemoveStudent(Service svc)
        {
            ...
            if (idx >= 1)
            {
                ...
                if (removed != null)
                {
                    svc.WriteToConsole($"   Студента «{removed.Name}» видалено (Причина: {reason}). Залишилось: {studentCount}");
                }
                else
                {
        ...
        public void HandleCalculateRating(Service svc)
        {
            ...
            if (idx >= 1)
            {
                ...
                else
                {
                    ...
                    svc.WriteToConsole($"  Рейтинг «{s.Name}: {r:F2} балів");
                }
            }
            else
            {
        ...
        public void HandleViewGrades(Service svc)
        {
            ...
            if (idx >= 1)
            {
                ...
                else
                {
                    ...
                    if (gs.Count == 0)
                    {
                        svc.WriteToConsole($"  У «{s.Name} оцінок немає.");
                    }
                    else
                    {
                        svc.WriteToConsole($"  Оцінки «{s.Name}»: [{string.Join(", ", gs)}]");
                    }
                }
            }
            else
        ...
        /// <summary>Додавання оцінок студенту.</summary>
        ...
        /// <summary>Зміна спеціальності студента.</summary>
            ...
            if (idx >= 1)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        svc.WriteToConsole($"   Спеціальність студента «{s.Name} змінено на «{newProgram}.");
                    }
                }
            }
            else
