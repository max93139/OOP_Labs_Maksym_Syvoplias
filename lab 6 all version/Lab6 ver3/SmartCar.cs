...
/// Він успадковує SmartDevice та реалізує обробку винятків у доменних методах (Reliability & Robustness).
/// </summary>
public sealed class SmartCar : SmartDevice
{
    ...
    private ScenarioResult MonitorDriverHealth(Service service, List<string> protocolLines)
    {
        ...
        catch (DriverImpairmentException ex)
        {
            ...
            LogLine(_chassis.ActivateEmergencyBraking(), service, protocolLines);
    ...
    private void PerformAILearningCycle(ScenarioData data, Service service, List<string> protocolLines)
    {
        try
        {
            ...
            {
                LogLine("Режим день: стандартне овітлення.", service, protocolLines);
            }
